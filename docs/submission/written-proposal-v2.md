...
At Festa Design Studio, we design for good. Our in-house projects directly informed RAPIDA's approach. JoinBody Geometry Topography Map Terrain — a production geospatial mapping system for Nigeria's cultural heritage — uses MapLibre GL with terrain overlays, 105 heritage site markers with sensitivity classifications (sacred sites use approximate ~5km locations), ethnicity boundary polygons, and multi-format data export (SVG, PDF, PNG). This project gave me direct experience with the exact geospatial stack that powers RAPIDA: MapLibre rendering, layered data overlays, feature markers with classification systems, and privacy-conscious location handling for sensitive sites. The Zika Alert App — a mobile-first PWA for real-time health alerts — uses the same Laravel Cloud architecture as RAPIDA with community-first design in a crisis context. Logframe — a visual workspace that turns theories of change into living project trackers — reflects the same structured impact thinking that shapes RAPIDA's incentivization system.
