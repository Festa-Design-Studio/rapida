<?php

return [
    'welcome_send_photo' => "مرحبًا بك في RAPIDA\nأرسل صورة للمبنى المتضرر للبدء.",
    'photo_received_send_location' => "تم استلام الصورة\n\nشارك موقعك (GPS) أو أرسل رمز what3words.",
    'please_send_photo' => 'يرجى إرسال صورة للمبنى.',
    'damage_options' => "ما مستوى الضرر؟\n\n1 طفيف\n2 جزئي\n3 كامل\n\nاقتراح الذكاء الاصطناعي: :suggestion",
    'infra_options' => "نوع البنية التحتية؟\n\n1 تجاري\n2 حكومي\n3 خدمات عامة\n4 نقل\n5 مجتمعي\n6 ترفيه\n7 أخرى",
    'crisis_type_options' => "نوع الأزمة؟\n\n1 زلزال\n2 فيضان\n3 تسونامي\n4 إعصار\n5 حريق\n6 انفجار\n7 نزاع",
    'debris_question' => "هل الحطام يسد الطريق؟\n\nأجب نعم أو لا",
    'confirm_summary' => "ملخص التقرير:\nصورة: تم الاستلام\nالموقع: :location\nالضرر: :damage\nالنوع: :infra_type\n\nاكتب نعم للإرسال.",
    'report_submitted' => "شكرًا. تقرير #:report_id تم إرساله!\n\nبياناتك تساعد الأمم المتحدة في تنسيق الاستجابة.",
    'session_restarted' => 'تم إعادة الجلسة. أرسل صورة للبدء.',
    'session_cancelled' => 'تم إلغاء التقرير. اكتب RAPIDA للبدء من جديد.',
    'max_retries_exceeded' => 'حدث خطأ عدة مرات. اكتب RAPIDA ونبدأ من جديد.',
    'confirm_or_restart' => 'اكتب نعم للإرسال أو إعادة للبدء.',
    'unknown_state' => 'انتهت جلستك بعد 30 دقيقة. اكتب RAPIDA عندما تكون مستعدًا.',
    'whatsapp_disabled' => 'هذه الأزمة تستخدم الإبلاغ المجهول فقط. يرجى استخدام التطبيق.',
    'location_conflict_mode' => 'صف مكان الضرر — اسم شارع أو معلم أو رمز من ثلاث كلمات.',

    // Hall lesson 12: inline language confirmation (gap-20)
    'language_inline_confirm' => '🇸🇦 سنواصل باللغة العربية. اكتب EN / FR / ES / ZH / RU للتغيير.',
];
